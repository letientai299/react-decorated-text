  ...
  return (
    <ul>
      <li>
        ...
        expression, e.g. <code>/p\w+g/ig</code>. Internally, the code construct
        the <code>RegExp</code> object by simple remove first <code>/</code>,
        then split the expression and flags at the last <code>/</code>.
      </li>
      <li>
        <strong>Options</strong>: Most option are simply an example usage of the
        provided <code>match</code> function, which will scan the text using the
        query to look for parts to apply different decorations. The exception is{' '}
        <code>typo</code>. It uses{' '}
        ...
        quite slow, will freeze the UI on the first time. If the query is not
        empty, <code>typo</code> will only spell-check those words match the
        query.
      </li>
    </ul>
  );
